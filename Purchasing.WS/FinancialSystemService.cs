...
namespace Purchasing.WS
{
    public class FinancialSystemService : IFinancialSystemService
    {
        ...
        public SubmitResult SubmitOrder(Order order, string userId, string kfsDocType = null)
        {
            ...
            {
                return new SubmitResult() { Success = false, Messages = new List<string>() { string.Format("{0} Exception: {1}", "There was an error communicating with the campus financial system.", ex.Message) } };
            }
            catch (Exception ex)
            {
